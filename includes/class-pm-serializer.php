...
class PM_Serializer {
    ...
    public function save() {
        ...
        if ( null !== wp_unslash( $_POST['pm-message'] ) ) {
 
            $value = sanitize_text_field( $_POST['pm-message'] );
            update_option( 'pm-recipient', $value );
    ...
    private function has_valid_nonce() {
        ...
        if ( ! isset( $_POST['pm-custom-message'] ) ) { // Input var okay.
            return false;
        }
 
        $field  = wp_unslash( $_POST['pm-custom-message'] );
        $action = 'pm-settings-save';
